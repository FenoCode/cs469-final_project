...
# Step 1.5: Balance the data to have equal number of 'ham' label and ['spam' + 'smish'] label
minority_count = len(df[df['label'].isin(['smish', 'spam'])])
ham_sample = df[df['label'] == 'ham'].sample(n=minority_count, random_state=42)
minority_df = df[df['label'].isin(['smish', 'spam'])]
df = pd.concat([ham_sample, minority_df])
...
axes[0].set_title('Confidence for Ham')
...
axes[1].set_title('Confidence for Smish/Spam')
